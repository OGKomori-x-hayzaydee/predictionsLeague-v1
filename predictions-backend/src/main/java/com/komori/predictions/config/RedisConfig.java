...
public class RedisConfig {
    @Bean
    public RedisTemplate<String, Fixture> redisFixtureTemplate(LettuceConnectionFactory lettuceConnectionFactory) {
    ...
    @Bean
    public RedisTemplate<String, Player> redisPlayerTemplate(LettuceConnectionFactory lettuceConnectionFactory) {
        RedisTemplate<String, Player> redisTemplate = new RedisTemplate<>();
        redisTemplate.setConnectionFactory(lettuceConnectionFactory);
        redisTemplate.setKeySerializer(new StringRedisSerializer());
        redisTemplate.setHashKeySerializer(new StringRedisSerializer());
        Jackson2JsonRedisSerializer<Player> serializer = new Jackson2JsonRedisSerializer<>(Player.class);
        redisTemplate.setValueSerializer(serializer);
        redisTemplate.setHashValueSerializer(serializer);
