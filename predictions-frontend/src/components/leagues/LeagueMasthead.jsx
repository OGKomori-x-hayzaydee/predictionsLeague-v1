  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-3 md:gap-4">
        ...
        <span
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-12 font-mono text-sm font-semibold tracking-[0.04em] md:h-[52px] md:w-[52px] md:rounded-14 md:text-base"
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0.5 md:gap-[2px]">
          <span className="truncate font-dmSerif text-xl leading-tight text-text-primary md:text-2xl">{overview?.name}</span>
          <span className="font-mono text-2xs tracking-[0.1em] text-text-muted-1 md:text-2xs">
        ...
        <div className="flex shrink-0 flex-col items-end gap-0.5 md:hidden">
          ...
          {move?.label && (
            <span className={`font-mono text-3xs ${move.tone === 'up' ? 'text-brand-teal' : move.tone === 'down' ? 'text-state-error' : 'text-text-muted-2'}`}>
              {move.label}
            </span>
          )}
        </div>
        <div className="hidden items-baseline gap-[11px] md:flex">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="font-mono text-2xs tracking-[0.1em] text-text-muted-4">{points} PTS</span>
            {move?.label && (
              <span className={`font-mono text-2xs ${move.tone === 'up' ? 'text-brand-teal' : move.tone === 'down' ? 'text-state-error' : 'text-text-muted-2'}`}>
                {move.label}
              </span>
            )}
          </div>
      ...
      <div className="relative flex items-baseline gap-[9px] border-t border-white/10 pt-[11px] md:hidden">
        ...
        <span className="font-mono text-2xs tracking-[0.1em] text-text-muted-1">POINTS</span>
      </div>

      <div className="relative hidden items-center gap-[22px] md:flex">
        <span className="flex min-w-0 flex-1 items-baseline gap-[9px]">
          <span className="font-mono text-2xs tracking-[0.12em] text-state-error">ABOVE</span>
          ...
          {neighbours.above && <span className="font-mono text-caption text-state-error">+{neighbours.gapAbove}</span>}
        </span>
        <span className="h-4 w-px bg-white/10" />
        <span className="flex min-w-0 flex-1 items-baseline gap-[9px]">
          <span className="font-mono text-2xs tracking-[0.12em] text-brand-teal">BELOW</span>
          ...
          {neighbours.below && <span className="font-mono text-caption text-brand-teal">−{neighbours.gapBelow}</span>}
        </span>
      </div>

      <span className="relative text-xs leading-relaxed text-text-secondary">{neighbours.verdict}</span>
    </div>
  );
}
