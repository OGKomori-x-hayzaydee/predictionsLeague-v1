    ...
    <div className="flex flex-col gap-[26px] px-4 py-6 md:px-[34px] md:py-[30px]">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between md:gap-6">
        <div className="flex flex-col gap-1.5 md:gap-[6px]">
          <h2 className="font-dmSerif text-[24px] leading-tight text-text-primary md:text-[34px]">
            {myLeagues.length > 0 ? `${myLeagues.length} league${myLeagues.length === 1 ? '' : 's'}, one record` : 'No leagues yet'}
          </h2>
          <p className="max-w-[44em] text-[12px] leading-relaxed text-text-muted-1 md:text-sm">
            {best
          ...
          <div className="grid grid-cols-2 gap-[9px] md:flex md:gap-[26px]">
            {stats.map((s) => (
              <div key={s.label} className="flex flex-col gap-[3px] rounded-13 border border-border-card bg-surface-card-2 p-3 md:items-end md:border-0 md:bg-transparent md:p-0">
                <span className="font-mono text-[10px] tracking-[0.13em] text-text-muted-4">{s.label}</span>
                <span className="font-dmSerif text-[22px] leading-none text-text-primary md:text-[26px]">{s.val}</span>
                <span className="text-[11px] text-text-muted-2">{s.note}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-[9px] md:grid-cols-2 md:gap-4">
          {myLeagues.map((l) => {
            ...
            return (
              ...
              >
                <span
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-12 font-dmSerif text-[17px] text-text-primary md:h-[46px] md:w-[46px]"
                  style={{ background: tone.tint, color: tone.var }}
                >
                  {l.name?.slice(0, 2).toUpperCase()}
                </span>
                <span className="flex min-w-0 flex-1 flex-col gap-[3px]">
                  <span className="truncate text-[14px] text-text-primary md:font-dmSerif md:text-[22px]">{l.name}</span>
                  <span className="font-mono text-[10px] text-text-muted-4 md:text-[10.5px]">
                    {l.members} member{l.members === 1 ? '' : 's'} · since {formatMonthYear(l.createdAt)}
                  </span>
                </span>
                <span className="flex shrink-0 flex-col items-end gap-[2px]">
                  <span className="font-dmSerif text-[21px] leading-none md:text-[30px]" style={{ color: tone.var }}>
                    {ordinal(l.position)}
                  </span>
                  <span className="font-mono text-[10px] text-text-muted-2">{l.points ?? 0} pts</span>
                </span>
              </button>
            );
          })}
          ...
          {!showForm ? (
            <button
              ...
              className="flex min-h-[72px] items-center justify-center gap-[9px] rounded-16 border border-dashed border-border-control text-[13.5px] text-text-muted-2 hover:border-brand-teal-mid/50 hover:text-text-primary"
            >
