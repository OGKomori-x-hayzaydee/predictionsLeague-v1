...
const GameweekChipsPanel = ({ 
  ...
  return (
    <div className="bg-gradient-to-br from-primary-600/40 to-primary-700/40 backdrop-blur-md rounded-lg border border-primary-400/20 p-4 mb-5">
      {/* Header - More compact with side-by-side layout - Now clickable */}
      <div 
        className="flex items-center justify-between mb-3 cursor-pointer"
        onClick={() => setIsPanelCollapsed(!isPanelCollapsed)}
      >
        <div className="flex items-center">
          <h2 className="text-teal-200 font-outfit font-medium text-xl mb-1">Chip Strategy</h2>
          <ChevronDownIcon 
            className={`ml-2 w-5 h-5 text-teal-300 transition-transform ${
              isPanelCollapsed ? 'transform rotate-180' : ''
            }`}
          />
        </div>
