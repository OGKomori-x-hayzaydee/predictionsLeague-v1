      ...
      <div className="relative">        {/* Header with icon and badge */}
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-2">            {icon && (
            ...
            <h3 className={`${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'} font-outfit font-medium text-xs`}>
              {title}
            </h3>
          </div>
            {badge && (
        ...
        <div className="flex items-baseline gap-2 mb-1.5">
          <p className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-800'} font-dmSerif`}>{value}</p>
          {trend && (
        ...
        {/* Subtitle */}
        <div className={`${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'} text-xs font-outfit`}>
          {subtitle}
        </div>
      </div>
    </motion.div>
