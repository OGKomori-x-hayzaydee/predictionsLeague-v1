...
const PredictionCarousel = ({
  ...
  const maxIndex = Math.max(0, predictions.length - itemsPerView);
    ...
    <div className="relative">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-1">
          {Array.from({ length: Math.ceil(predictions.length / itemsPerView) }).map((_, index) => (
            <button
              key={index}
      ...
      <div className="mt-4 text-center">
        <span className={`text-sm ${theme === "dark" ? "text-slate-400" : "text-slate-500"}`}>
          {currentIndex + 1}-{Math.min(currentIndex + itemsPerView, predictions.length)} of {predictions.length}
        </span>
      </div>
    </div>
  );
