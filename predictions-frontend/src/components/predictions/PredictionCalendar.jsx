...
const PredictionCalendar = ({
  ...
  }, [predictions, searchQuery, isLeagueMode]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`grid grid-cols-1 ${isLeagueMode ? "" : "lg:grid-cols-4"} gap-5`}>
        {/* Calendar grid */}
        <div className={isLeagueMode ? "" : "lg:col-span-2"}>
          {/* Weekday headers */}
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarDays.map((day, index) => {
              ...
              if (isLeagueMode) {
                // League mode: expanded cells with userDisplayName pills
                return (
                  <div
                    key={index}
                    ...
                    {hasPredictions && (
                      <div className="space-y-0.5">
                        {dayPredictions.slice(0, 2).map((prediction, idx) => (
                          <div
                            key={idx}
                            className={`text-2xs p-0.5 rounded truncate ${
                              theme === 'dark'
              ...
              // Personal mode: compact cells with dot indicators
              return (
                <motion.button
                  key={index}
            ...
            <div>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {selectedDatePredictions.map((prediction, index) => (
                  <motion.div
                    key={prediction.id || `${prediction.userId}-${prediction.fixtureId}-${index}`}
                  ...
                  >
                    <PredictionCard
                      ...
                      mode={mode}
                      showMemberInfo={isLeagueMode || showMemberInfo}
                      onSelect={onPredictionSelect}
                      onEdit={isLeagueMode ? undefined : onEditClick}
                      isReadonly={isLeagueMode || isReadonly}
                      size={cardStyle}
                    />
                  </motion.div>
                ))}
