    ...
    <div className="flex flex-col gap-[13px]">
      ...
      <div className="flex flex-col gap-2.5">
        {(ledger || []).map((entry) => (
          ...
          >
            <span className="h-[26px] w-[2px] shrink-0 rounded-full" style={{ background: entry.mark }} />
            <span className="flex min-w-0 flex-1 flex-col leading-[1.25]">
              <span className="truncate text-sm text-text-secondary">{entry.match}</span>
              <span className="font-mono text-[11px] text-text-muted-3">{entry.detail}</span>
            </span>
      ...
      {footer && <span className="font-mono text-[11px] text-text-muted-4">{footer}</span>}
    </div>
  );
}
